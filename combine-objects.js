// Your task is to write a function that takes two or more objects and returns a new object which combines 
...
function combine(...objs) {
    return objs.reduce((acc, cv) =>{
        for(let p in cv){
            if(cv.hasOwnProperty(p))
                acc[p] = (acc[p] || 0) + cv[p]
        }
    return acc    
    }, {})
