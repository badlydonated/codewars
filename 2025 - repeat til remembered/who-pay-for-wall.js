...
// Return - an array containing the full string and a truncated one
// Examples - "Mexico" becomes ["Mexico", "Me"]
// Psuedocode - check if str is <= 2.  If it is, return an array with the str as is.  
//              if it is not - split the str to create array of letters 
...
function whoIsPaying(name){
  if(name.length <= 2){
    ...
    let name2 = name.split('').slice(0,2).join('') // didn't need the new variable
    return [name, name2]
  }
}
